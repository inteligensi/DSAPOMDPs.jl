function state_sub2ind(dims::Tuple, i1, i2, i3, i4, i5)
    return ((i5 - 1) * prod(dims[1:4]) +
            (i4 - 1) * prod(dims[1:3]) +
            (i3 - 1) * prod(dims[1:2]) +
            (i2 - 1) * dims[1] +
             i1)
...
function POMDPs.stateindex(P::DSAPOMDP, s::State)
    ...
    hypertension_idx = s.hypertension ? 1 : 2
 
    return state_sub2ind((2, 2, 2, P.max_duration + 1, 2), ane_idx, avm_idx, occ_idx, time_idx, hypertension_idx)
 end
...
function Distributions.pdf(dist::Union{Distributions.ProductDistribution}, o::Observation)
    o_values = [o.is_ane, o.is_avm, o.is_occ]
